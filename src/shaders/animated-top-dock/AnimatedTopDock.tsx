...
export type AnimatedTopDockProps = {
  ...
  onItemActivate?: (id: string, event: MouseEvent<HTMLButtonElement>) => void;
  activeId?: string;
  showGhost?: boolean;
  showStage?: boolean;
  showBrand?: boolean;
...
export function AnimatedTopDock({
  ...
  const items = dockItemsProp ?? VARIANT_ITEMS[variant] ?? ITEMS;
  const [activeInternal, setActive] = useState(items[0]?.id ?? "");
  const active = activeIdProp ?? activeInternal;
  const isHeader = shell === "header";
  const showStageContent = showStage ?? shell === "demo";
  ...
  const handleItemActivate = (id: string, event: MouseEvent<HTMLButtonElement>) => {
    if (onItemActivate) {
      onItemActivate(id, event);
    } else {
      setActive(id);
    }
  };
  ...
  const dockItems = (itemClass: string, iconClass: string, viewBox: string) => items.map((item) => (
    <button
      ...
      onClick={(event) => handleItemActivate(item.id, event)}
    >
  ...
  if (variant === "modern") {
    const resolvedBrandWord = brandWord ?? (isHeader ? undefined : "Lumina");
    const resolvedGhostLabel = ghostLabel ?? "Sign in";
    const resolvedCtaLabel = ctaLabel ?? "Start building";

    return (
      <div className={`animated-top-dock-component atd-modern${isHeader ? " atd-modern--header" : ""}${isHeader && !showBrand ? " atd-modern--no-brand" : ""}${className ? ` ${className}` : ""}`}>
        {!isHeader && <div className="atd-modern__aurora" aria-hidden="true" />}
        <header className="atd-modern__bar">
          {showBrand && (
            <a className="atd-modern__brand" href="#top-dock" onClick={handleBrandClick}>
              <span className="atd-modern__mark" aria-hidden="true">{brandMark ?? BRAND_MARK}</span>
              {resolvedBrandWord ? <span className="atd-modern__word">{resolvedBrandWord}</span> : null}
            </a>
          )}
        ...
        {!isHeader && <p className="animated-top-dock-component__caption">LOGO LEFT · DOCK CENTRE · ACTION RIGHT</p>}
      </div>
    );
  }
  ...
  if (variant === "retro") {
    return (
      <div
        ...
        style={{ "--atd-retro-scan": optionsRef.current.scanlines } as CSSProperties}
      >
    ...
    return (
      <div ref={glass.hostRef} className={`animated-top-dock-component atd-glass${className ? ` ${className}` : ""}`} data-dock-frame>
        ...
        <header className="atd-glass__rail">
          <a className="atd-glass__brand" href="#top-dock" onClick={handleBrandClick}>
            <span className="atd-glass__mark" aria-hidden="true">{brandMark ?? BRAND_MARK}</span>
            <span className="atd-glass__word">{brandWord ?? "Aperture"}</span>
          </a>
  ...
  return (
    <div className={`animated-top-dock-component${className ? ` ${className}` : ""}`}>
      <nav ref={rootRef} className="animated-top-dock__nav" aria-label="Animated top dock" data-dock-state="idle" data-dock-max="0.00">
        <button className="animated-top-dock__item animated-top-dock__logo" data-dock-item type="button" aria-label="Home" onClick={(event) => handleItemActivate("system", event)}>
          {brandMark ?? BRAND_MARK}
        </button>
        {items.map((item) => (
          <button key={item.id} className="animated-top-dock__item animated-top-dock__link" data-dock-item type="button" aria-pressed={active === item.id} onClick={(event) => handleItemActivate(item.id, event)}>
